...
{         
          If([string]::IsNullOrEmpty($csvItem.SOURCE_MACHINE_NAME)){
             Write-Error "TARGET_MACHINE_NAME is Empty!" -TargetObject $_}
          ...
          Else{$RecoveryVaultName=$csvItem.VAULT_NAME}
            ...
            $replicatedVM = Get-AzRecoveryServicesAsrReplicationProtectedItem -FriendlyName $VMName -ProtectionContainer $ProtectionContainer -ErrorAction SilentlyContinue
            ...
            $tempASRJob = Set-AzRecoveryServicesAsrReplicationProtectedItem -InputObject $replicatedVM -Size $TargetVMSize -UseManagedDisk True -LicenseType NoLicenseType
	...
	{
        LogError "Exception in Test Failover"
